<script>
    // @ts-nocheck
      ...
      const id = `${title}-${functions}-${battery}`; // or pass `id` directly from ProductGrid
      ...
      const product = {
        ...
        price,
        thumbnail: image,
        description: `${functions} Functions / ${battery} Battery`,
      };
      ...
      <div class="w-1/2 p-6 flex flex-col justify-between">
        <div>
          ...
          <div class="mt-3 space-y-2 text-base">
            <p class="flex justify-between"><span>Function</span> <span class="font-semibold">{functions}</span></p>
