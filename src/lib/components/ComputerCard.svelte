<script>
    // @ts-nocheck
      ...
      const id = `${title}-${cpu}-${ram}`; // or pass `id` directly from ProductGrid
      ...
      <div class="w-1/2 p-6 flex flex-col justify-between">
        <div>
          ...
          <div class="mt-3 space-y-2 text-base">
            <p class="flex justify-between"><span>CPU</span> <span class="font-semibold">{cpu}</span></p>
            <p class="flex justify-between"><span>RAM</span> <span class="font-semibold">{ram}</span></p>
