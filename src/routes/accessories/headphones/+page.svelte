<!-- src/routes/accessories/earbuds/+page.svelte -->
<script lang="ts">
  import { onMount } from 'svelte';
  import EarbudsCard from '$lib/components/EarbudsCard.svelte';
  ...
  // Filter products by category ID (earbuds)
  const EARBUDS_CATEGORY_ID = 6; // Adjust this based on your database

  // Helper function to get bluetooth and battery from specs OR direct fields
  function getSpecValue(product: Product, specName: string): string {
    // First check if it's stored directly in the product
    if (specName.toLowerCase() === 'bluetooth' && product.bluetooth) {
    ...
    const spec = product.specs.find(s => 
      ...
      s.name.toLowerCase().includes('connectivity') && specName.toLowerCase() === 'bluetooth' ||
      s.name.toLowerCase().includes('battery life') && specName.toLowerCase() === 'battery'
    );
  ...
  function getImageUrl(product: Product): string {
    ...
    return '/placeholder-earbuds.jpg';
  }
  ...
  async function loadProducts(page: number = 1) {
    try {
      ...
      const response = await productApi.getProducts(page, EARBUDS_CATEGORY_ID);
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-5 px-4">
      {#each filteredProducts as product (product.id)}
        <EarbudsCard
          ...
          bluetooth={getSpecValue(product, 'bluetooth')}
