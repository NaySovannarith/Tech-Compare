<!-- src/routes/accessories/smartwatches/+page.svelte -->
<script lang="ts">
  ...
  function getSpecValue(product: Product, specName: string): string {
    ...
    if (specName.toLowerCase() === 'functions' && product.functions) {
      return product.functions;
    }
    ...
    const spec = product.specs.find(s => 
      ...
      s.name.toLowerCase().includes('feature') && specName.toLowerCase() === 'functions' ||
      s.name.toLowerCase().includes('life') && specName.toLowerCase() === 'battery'
    );
...
function getImageUrl(product: Product): string {
  ...
  async function loadProducts(page: number = 1) {
    try {
      ...
      const response = await productApi.getProducts(page, SMARTWATCH_CATEGORY_ID);
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-5 px-4">
      {#each filteredProducts as product (product.id)}
        <SmartwatchCard
          ...
          functions={getSpecValue(product, 'functions')}
