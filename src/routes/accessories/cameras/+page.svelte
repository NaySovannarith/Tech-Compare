<!-- src/routes/accessories/cameras/+page.svelte -->
<script lang="ts">
  ...
  function getSpecValue(product: Product, specName: string): string {
    ...
    if (specName.toLowerCase() === 'resolution' && product.resolution) {
    ...
    const spec = product.specs.find(s => 
      ...
      s.name.toLowerCase().includes('resulution') && specName.toLowerCase() === 'resolution' ||
...
function getImageUrl(product: Product): string {
  ...
  async function loadProducts(page: number = 1) {
    try {
      ...
      const response = await productApi.getProducts(page, CAMERA_CATEGORY_ID);
