...
<div class="  mt-[95px] bg-gray-100 min-h-screen">
  <div class=" mt-[95px] flex items-center justify-between mb-4 p-4">
    <button
      on:click={goBack}
    ...
    <h1 class="text-2xl font-bold text-center flex-1 mb-5 mt-6">
      My Wishlist (Test)
    </h1>

    <div class="w-[112px]"></div>
  </div>
  <p class="text-xl text-center flex-1 mb-8 mt-4">
    Save your faverite products with our WishList Feature.
  </p>
      ...
      <ul class="space-y-4">
        {#each $wishlist as product (product.id)}
          <li class="container01">
            ...
            <div
              class="w-1/4 flex flex-col items-center justify-center text-center"
            >
            ...
            <!-- Stock + Actions -->
            <div
              class="w-1/4 flex flex-col items-end justify-center text-right gap-1"
            >
              ...
              <button
                ...
                on:click={() =>
                  goto(`/reviews/${encodeURIComponent(product.title)}`)}
              >
                Leave a Review
              </button>
...
<style>
  .container01 {
    ...
    border: 1px solid #e5e7eb; /* Tailwind's default border color (light gray) */
    padding: 1rem; /* 1rem = 16px */
    border-radius: 0.25rem; /* 4px rounded corners */
    box-shadow:
      0 1px 3px 0 rgba(0, 0, 0, 0.1),
      0 1px 2px 0 rgba(0, 0, 0, 0.06); /* Tailwind's default shadow */
